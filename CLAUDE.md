...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-12: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is ~35% done** — public signup, admin-created accounts with a temporary password, student and people listings, and real role-based authorization in the backend. Next is `inscripcion`. Current state and open questions live in `docs/sistema-gestion-plan.md` §6d.
