...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-15: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is ~35% done** — public signup, admin-created accounts with a temporary password, student and people listings, real role-based authorization, and `AlumnoTest` covering the module's own logic. **Next is `inscripcion`**: the table exists since `V1`, every rule it needs is settled, and nothing blocks it. The starting point — what to build, in what order, and which rules the database already enforces for you — is `docs/sistema-gestion-plan.md` §6d.
...
>
> **Next is `inscripcion`, and nothing blocks it.** Start at `docs/sistema-gestion-plan.md` §6d.
