...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-16: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is closed** — all four of its screens exist: people and student listings (the latter filtering by discipline and course level), alta/edición, the student profile at `/admin/alumnos/:id`, and inscriptions at `/admin/inscripciones`. **Módulo 2 (Horarios y salas) is closed**: the whole backend plus its four screens — the weekly calendar at `/admin/reservas`, room blocking at `/admin/bloqueos`, and the usage report at `/admin/uso-salas`. The one thing it still owed, the deposit, moved to Módulo 3 because its trigger needs a `pago` row. **Módulo 3 (Pagos) is under way**: `pago` and `egreso` backends plus five of its six screens — `/admin/pagos`, `/admin/estado-de-cuenta/:idUsuario`, `/admin/caja`, `/admin/deudores` and `/admin/egresos`. What it still owes is the deposit migration (`V10`) and the equipment-sales sub-section. Suites are at **278 backend / 210 platform**. **Módulo 1's profile now builds five of its six blocks**: class history and account balance were filled in on 2026-08-16 once modules 2 and 3 existed. Only teacher notes/materials is still drawn as a named placeholder, waiting on `nota_profesor` and Módulo 5. The starting point is `docs/sistema-gestion-plan.md` §6d.

> **The audit remediation is closed** (2026-08-15): all eight batches done, **56 of 56 resolved, backlog at zero**, and **nothing is blocked on a decision from Ignacio any more** — the deposit was the last one. Of the 61 findings originally raised, one is an accepted risk and four stopped being audit work and moved to the document where they can actually be done: DOC-08 and QA-07 (deploy) to `docs/operacion.md` §3, and **DB-04 and DB-11 to `docs/requirements/platform.md` §5, as part of finishing Módulo 2**. Those four do not close by themselves.
>
> **The one thing still worth carrying forward:** of the two business rules that lived in a document rather than in the code, **DB-11 (the hour order) was implemented on 2026-08-16** as an `@AssertTrue` on the reservation DTOs. **The deposit is now the only one left.** Its blocker is gone — Módulo 3 creates `pago` rows — but writing the trigger surfaced a design clash: §13 assumed a reservation and its payment enter in one transaction, and Módulo 2 deliberately splits them (the alta creates the `reserva`, participants are added in a separate request). **Decided 2026-08-16: adapt the flow, not the rule** — the class alta will create the reservation and its participant together, and `V10` comes last. The three-step order is in `platform.md` §13; start there, not at the migration.
>
> **Módulo 1 and Módulo 2 both closed on 2026-08-16. Módulo 3 (Pagos) is next, and it starts owing the deposit rule that Módulo 2 handed it.** Start at `docs/sistema-gestion-plan.md` §6d.
...
cd apps/platform && npm test          # vitest, 210 cases, no backend needed
...
**The deposit rule (DB-04a) was decided on 2026-08-15 and is the last rule the schema still owes.** Ignacio's words: *"everything must be deposited up front, everything — except mix & mastering, which Ghezz decides case by case. The deposit is 50% of the total."* That matches what §1 already had confirmed from the client interview (*"no deposit, no slot"*, and M&M being fielded on credit). The full ruling, including how it maps onto the schema, is in `docs/requirements/platform.md` §13.

Two things about implementing it, because neither is obvious:

- **"Everything" does not mean every `reserva` row needs its own payment.** It means no reservation exists without money behind it, and that money arrives by one of two routes: a `pago` pointing at the reservation (booth rental, set recording), **or the inscription that covers the class**, through `reserva_participante.id_inscripcion`. A student who paid the whole course up front does not owe a deposit per class — that would be charging twice, and it contradicts §13's *"the course is paid in full before starting"*.
- **The 50% is checkable on an inscription and not yet on a reservation.** `inscripcion.precio_total` exists; **`reserva` has no price** — a rental's price comes from hours × a rate that isn't in the system yet (P13, Módulo 3). Until that exists, the database can require *that a payment exists* but not *that it is half*. The screen enforces that half; the database takes it over when `reserva` has a price.

`V9`'s header records the tool (a `CONSTRAINT TRIGGER … DEFERRABLE INITIALLY DEFERRED` checking at COMMIT) and why it wasn't guessed at. **The migration goes with the start of Módulo 2**, not before: the trigger forces a reservation and its payment into the same transaction, which is a condition on screens that don't exist yet.
