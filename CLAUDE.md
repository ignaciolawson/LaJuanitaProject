...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-16: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is ~75% done** — public signup, admin-created accounts with a temporary password, student and people listings, real role-based authorization, and **`inscripcion` complete end to end**: backend (alta, listing with four filters, edit, state change, computed remaining classes) *and* its screen at `/admin/inscripciones`. Suites are at **151 backend / 72 platform**. **What's left of Módulo 1**: the student profile, and discipline/level filters on the *alumnos* listing itself (which needs a backend join — the Inscripciones screen answers "who is taking DJ", not "filter the student list by discipline"). The starting point is `docs/sistema-gestion-plan.md` §6d.
...
>
> **`inscripcion` is done end to end (2026-08-16), screen included. What's next is the student profile, and nothing blocks it.** Start at `docs/sistema-gestion-plan.md` §6d.
...
cd apps/platform && npm test          # vitest, 72 cases, no backend needed
