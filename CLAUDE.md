...
cd apps/platform && npm test          # vitest, 231 cases, no backend needed
...
**Database rule tests** — 177 cases (127 business-rule + 50 adversarial) covering every
...
Postgres service — which also proves the ten migrations apply to an empty database, the
...
**Deliberate hole, documented in `V10`'s header the way `V6` and `V7` document theirs:** the trigger runs **only on `INSERT` of `reserva`**, so the invariant is established at creation and can be broken afterwards by annulling the payment. Closing it is a Módulo 3 decision about refunds — annulling to correct and annulling to walk away are the same UPDATE — not this migration's.
...
**Three things Módulo 3 leaves open on purpose, and they are one decision: don't build an irreversible operation nobody asked for.** (1) `egreso` and `venta_equipo` **cannot be annulled or corrected** — `V9` gave them the annulment columns, but that was to justify banning `DELETE`; the screen arrives when someone asks. A mis-entered sale or expense stays. (2) **A sale loaded without payment has no later way to be collected**, since `/admin/pagos` still only settles inscriptions and accepting the other destination means rebuilding that form. (3) The deposit hole above.
