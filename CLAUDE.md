...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-16: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is ~85% done** — public signup, admin-created accounts with a temporary password, people and student listings (the latter now filtering by discipline and course level), real role-based authorization, and **`inscripcion` complete end to end**: backend and its screen at `/admin/inscripciones`. Suites are at **159 backend / 76 platform**. **The only thing left in Módulo 1 is the student profile**, and it needs no new backend. The starting point is `docs/sistema-gestion-plan.md` §6d.
...
>
> **`inscripcion` is done end to end (2026-08-16), screen and student-list filters included. What's next is the student profile, and nothing blocks it.** Start at `docs/sistema-gestion-plan.md` §6d.
...
cd apps/platform && npm test          # vitest, 76 cases, no backend needed
