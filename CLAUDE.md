...
> **Three things to know before starting anything, all in `docs/sistema-gestion-plan.md` §6d:** everything is pushed but **the CI pipeline has still never been seen running on Actions** — its four steps were only ever verified locally; **the deposit rule (DB-04a) is undecided and blocks Módulo 2**; and **the audit remediation is finished** — **56 of 61 resolved, nothing open**, all eight batches closed on 2026-08-15. The four that remain are half-done and none of them is code: they wait on the deposit decision, on October's hosting, and on a Module 2 DTO that doesn't exist yet.
