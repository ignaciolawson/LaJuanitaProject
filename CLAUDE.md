...
>
> **The one thing still worth carrying forward:** of the two business rules that lived in a document rather than in the code, **DB-11 (the hour order) was implemented on 2026-08-16** as an `@AssertTrue` on the reservation DTOs. **The deposit is now the only one left**, and it moved to Módulo 3 — its trigger needs a `pago` row, and no module can create one yet. See `platform.md` §5.
>
...
cd apps/platform && npm test          # vitest, 197 cases, no backend needed
