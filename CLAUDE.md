...
>
> **Módulo 3 closed on 2026-08-17. Next is Módulo 4.** Start at `docs/sistema-gestion-plan.md` §6d.
...
cd apps/platform && npm test          # vitest, 241 cases, no backend needed
...
**Database rule tests** — 188 cases (138 business-rule + 50 adversarial) covering every
...
Postgres service — which also proves the twelve migrations apply to an empty database, the
...
**`@Column(insertable = false)` is not enough to return a database DEFAULT.** Hibernate never re-reads the column after the INSERT, so the row it hands back has `null` there. `BloqueoSala.fechaRegistro` needs `@Generated(event = EventType.INSERT)` because the alta echoes the created row and the screen shows when it was loaded. The four other entities with a creation stamp use the same bare `insertable = false` and are **not** broken — none of them exposes it in a DTO, which is why it never showed up. Adding one to a DTO means adding the annotation.
