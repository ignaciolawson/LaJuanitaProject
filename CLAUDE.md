...
2. **Management platform** (`apps/platform` + `apps/backend`) — authenticated system covering students, room/schedule booking, payments, student/teacher portals, mix & mastering requests, and the record label workflow. **This is the current work.** As of 2026-08-16: Fase 0 is closed (database, JWT login, protected routes, role-driven menu) and **Módulo 1 (Alumnos) is ~55% done** — public signup, admin-created accounts with a temporary password, student and people listings, real role-based authorization, and **`inscripcion` complete on the backend** (alta, listing with four filters, edit, state change, computed remaining classes; `InscripcionTest`, 39 cases, suite now at 147). **What's left of Módulo 1 is front-end work**: the inscription screen, the discipline/level filters it unblocks, and the student profile. The starting point is `docs/sistema-gestion-plan.md` §6d.
...
>
> **`inscripcion`'s backend is done (2026-08-16); what's next is its screen, and nothing blocks it.** Start at `docs/sistema-gestion-plan.md` §6d.
