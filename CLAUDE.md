...
> **Two things to know before starting anything, both in `docs/sistema-gestion-plan.md` §6d:** everything is pushed but **the CI pipeline has still never been seen running on Actions** — its four steps were only ever verified locally; and **the audit remediation is finished** — **56 of 59 resolved, nothing open**, all eight batches closed on 2026-08-15. The two that remain both wait on Módulo 2, not on a decision: the deposit migration (DB-04, decided 2026-08-15) and a reservation DTO that doesn't exist yet (DB-11). *(59 and not 61: DOC-08 and QA-07 — both deploy — were taken out of the backlog the same day; that work is tracked in `docs/operacion.md` §3, not here.)*
>
> **Nothing is blocked on a decision from Ignacio any more.** The deposit was the last one.
