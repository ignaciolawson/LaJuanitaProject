...
> **Three things to know before starting anything, all in `docs/sistema-gestion-plan.md` §6d:** everything is pushed but **the CI pipeline has still never been seen running on Actions** — its four steps were only ever verified locally; **the deposit rule (DB-04a) is undecided and blocks Módulo 2**; and **the audit remediation is finished** — **56 of 59 resolved, nothing open**, all eight batches closed on 2026-08-15. Two remain and neither is code: the deposit decision (DB-04) and a Module 2 DTO that doesn't exist yet (DB-11). *(59 and not 61: DOC-08 and QA-07 — both deploy — were taken out of the backlog the same day; that work is tracked in `docs/operacion.md` §3, not here.)*
