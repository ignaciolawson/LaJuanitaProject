...
**Backend**: Spring Boot 4.1 / Java 21, Spring Data JPA, Spring Security (JWT), Bean Validation, Lombok. `spring.jpa.hibernate.ddl-auto=validate` — no auto-DDL, ever. Schema lives in Flyway migrations under `src/main/resources/db/migration`: `V1__baseline.sql` (22 tables), `V2__datos_iniciales.sql` (rooms + the room×use matrix), `V3__usuario_admin_inicial.sql` (dev admin), `V4__separar_nombre_apellido.sql`, `V5__cambio_de_password_obligatorio.sql` and `V6__integridad_auditoria.sql` — the last one closes 10 integrity holes found by attacking the running schema (12 CHECKs, 3 triggers, an EXCLUDE and a partial UNIQUE); the attacks and what was deliberately left open are written up in **`docs/db/auditoria-2026-08-12.md`**, which is the companion document to that migration. As of 2026-08-13 all six apply cleanly to an empty database and both SQL suites pass on the result (69/69 and 40/40).
