...
> **Three things to know before starting anything, all in `docs/sistema-gestion-plan.md` §6d:** there are **commits that were never pushed** and a CI pipeline that has never run on Actions; **the deposit rule (DB-04a) is undecided and blocks Módulo 2**; and the audit backlog is down to **42 of 61 resolved**, with everything still open being tanda 8 cleanup that blocks nothing.
