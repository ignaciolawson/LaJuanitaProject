...
>
> **Módulo 5's backend landed on 2026-08-19; its front end does not exist.** The module is *not* closed — this is the first one left split in half, and it is exactly what closing Módulo 2 taught not to assume: *"a module isn't closed because both halves have tests"*. Here there aren't two halves. **`docs/sistema-gestion-plan.md` §6d lists screen by screen what is missing and which endpoint each one already has.** Start there.
>
...
**A screen that can't be finished says so instead of omitting the part it lacks.** The student profile needs six blocks and can build two; the other three are rendered as a named "todavía no disponible" list with the module that brings each. Same reasoning as `menu.ts`'s `disponible: false`: a missing block reads as *the system lost the data*, a labelled one reads as *not built yet*. Don't "clean up" those placeholders — deleting them makes the screen look complete when it isn't.
...
**Módulo 5's backend (2026-08-19, `docencia` package, `V14`) — the front end is missing, see §6d.** What it decided:
