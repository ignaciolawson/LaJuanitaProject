...
>
> **Módulo 1 closed on 2026-08-16, and Módulo 2 is nearly there: backend plus the weekly calendar at `/admin/reservas`. What's left of it is room blocking and the usage report.** Start at `docs/sistema-gestion-plan.md` §6d.
...
cd apps/platform && npm test          # vitest, 103 cases, no backend needed
...
**Módulo 2's backend exists as of 2026-08-16** (`reserva`, `sala` packages): rooms and use types with the §2.6 matrix, the agenda by date range, reservations, participants and attendance. Four decisions were settled that day — **classes are loaded by hand, one at a time** (P7: auto-generating the 8 weekly ones was the document's own recommendation and was rejected), **a class does not require an assigned teacher** (P37), DB-11 is done as an `@AssertTrue` on the reservation DTOs, and **the deposit trigger moved to Módulo 3** because it needs a `pago` row that no module can create yet.
