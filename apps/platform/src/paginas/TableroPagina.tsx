    ...
    <div>
      ...
      <Seccion titulo="La caja del período" detalle={`/admin/caja?${rango}`}>
        <div className="grid gap-4 sm:grid-cols-2">
          {caja.map((c) => (
            <div key={c.moneda} className="rounded-lg border border-linea bg-superficie shadow-tarjeta p-5">
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          {pendientes.map((p) => (
            <div key={p.moneda} className="rounded-lg border border-linea bg-superficie shadow-tarjeta p-5">
              <div className="flex items-baseline justify-between">
          ...
          <Seccion titulo="De dónde salió la plata" aclaracion="Del período elegido">
            <div className="grid gap-4 sm:grid-cols-2">
              {(['ARS', 'USD'] as const).map((moneda) => {
                ...
                return (
                  <div key={moneda} className="rounded-lg border border-linea bg-superficie shadow-tarjeta p-5">
                    <div className="flex items-baseline justify-between">
